...
def train(net_name=_net_name,
    ...
    with tf.Session(config=config) as sess:
        ...
        # Train model
        try:
            pbar = tqdm.tqdm_notebook(range(num_batches), smoothing=0.02, ncols='100%')
            loss, vloss = 0, 0
            for batch in pbar:
                if batch > pre_train_steps and batch % summary_step == 0:
                    ...
                    pbar.set_description('Loss: {:.2E} Validation: {:.2E}'.format(loss, vloss))
                ...
                if batch > pre_train_steps and batch % validation_step == 0:
                    flags = dict({'testing_flag:0': True})
                    vloss, summaries, accuracy = sess.run([pred_loss, merged_with_imgs, loss_over_time],
                                                          feed_dict=flags)
                    for summary in summaries:
                        test_writer.add_summary(summary, batch + starting_batch)
