...
def main():
    ...
    with tf.Session() as sess:
        ...
        test_writer = tf.summary.FileWriter(J('.',saveDir,'test', 'alpha-' + str(a) + 'beta-' + str(b) + 'gama-' + str(g)), sess.graph)
        ...
        for epoch in range(train_epoch+1):
            ...
            if epoch > pre_train_steps:
                if epoch % summary_step == 0:
                    ...
                    [writer.add_summary(sess.run([summary],feed_dict=test_dic)[0], epoch) for (writer,summary) in zip(writers, summary_list)]
